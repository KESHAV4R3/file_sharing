...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen flex bg-slate-950 text-slate-100">
      {/* Sidebar */}
      <aside className="w-60 shrink-0 border-r border-slate-800/80 bg-slate-900/50 backdrop-blur-sm flex flex-col">
        {/* Brand */}
        <div className="px-5 py-5 border-b border-slate-800/60">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-violet-600 to-indigo-500 p-0.5 shadow-lg shadow-violet-500/20">
      ...
      <main className="flex-1 overflow-auto">
        {/* Top bar */}
        <header className="sticky top-0 z-10 px-8 py-4 bg-slate-950/80 backdrop-blur-md border-b border-slate-800/60 flex items-center justify-between">
          <div>
            <h1 className="text-lg font-bold text-white">
              {selectedUser ? 'User Detail' : view === 'overview' ? 'Overview' : 'All Users'}
            </h1>
            <p className="text-xs text-slate-500">DocReader Control Center</p>
          </div>
        </header>
